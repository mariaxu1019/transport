...
class StudentUSocket(StudentUSocketBase):
  ...
  def connect(self, ip, port):
    ...
    ## Start of Stage 1.1 ##
    syn_pkt = self.new_packet(ack=False, data=None, syn=True)
    syn_pkt.tcp.seq = self.snd.iss
    self.tx(syn_pkt)
    self.state = SYN_SENT
    ## End of Stage 1.1 ##
  ...
  def update_rto(self, acked_pkt):
    ...
    if self.srtt == 0 and self.rttvar == 0:
      ...
      self.rto = max(self.MIN_RTO, min(self.MAX_RTO, self.srtt + max(self.G, self.K * self.rttvar)))
    ## End of Stage 9.1 ##
  ...
  def check_ack(self, seg):
    ...
    if self.state in (ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2, CLOSE_WAIT, CLOSING):
      ## Start of Stage 4.1 ##
      is_unacked = snd.una |LE| seg.ack
      is_sent = seg.ack |LE| snd.nxt
      if is_unacked and is_sent: 
  ...
  def maybe_send(self):
    ...
    remaining = (self.snd.una + self.snd.wnd) - self.snd.nxt
    while len(self.tx_data) > 0 and remaining > 0:
      num_pkts += 1
      end = min(self.mss, remaining, len(self.tx_data))
      payload = self.tx_data[:end]
      bytes_sent += len(payload)
      self.tx_data = self.tx_data[end:]
